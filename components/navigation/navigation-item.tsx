...
const NavigationItem = ({id,imageUrl,name}:NavigationItemProps) => {
    ...
    <ActionTooltip side="right" align="center" label={name}>
        <button onClick={()=>{router.push("/servers/"+id)}} className=" group relative flex items-center">
            ...
            <div className={cn(" relative group flex mx-3 h-[48px] w-[48px] rounded-[24px] group-hover:rounded-[16px] transition-all overflow-hidden", params?.serverId == id && " bg-primary/10 text-primary rounded-[16px]")}><Image fill src={`http://localhost:7000/`+imageUrl} alt="channel"/></div>
        </button>
    </ActionTooltip>
    );
}
